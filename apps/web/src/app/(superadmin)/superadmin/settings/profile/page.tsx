...
export default async function SuperadminProfilePage() {
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6">
      ...
      >
        <ArrowLeft className="h-4 w-4" /> {t("partner.profile.backToSuperadminSettings")}
      </Link>
      <div>
        <h1 className="text-2xl font-bold text-foreground">{t("partner.profile.title")}</h1>
        <p className="mt-0.5 text-sm text-muted-foreground">{t("partner.profile.subtitleSuperadmin")}</p>
      </div>
